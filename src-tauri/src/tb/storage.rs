...
pub struct TbDb { conn: Connection }
...
impl TbDb {
    pub fn init(tb_id: &str, name: &str) -> Result<()> {
        ...
        ")?;
        conn.execute("INSERT OR REPLACE INTO meta VALUES ('name', ?1)", params![name])?;
        Ok(())
    }

    pub fn open(tb_id: &str) -> Result<Self> {
        Ok(Self { conn: Connection::open(db_path(tb_id))? })
    }
    ...
    pub fn search(&self, term: &str, source_lang: &str) -> Result<Vec<TbEntry>> {
        ...
        let entries = stmt.query_map(params![source_lang, pattern], |row| Ok(TbEntry {
            id: row.get(0)?, source_term: row.get(1)?, target_term: row.get(2)?,
            source_lang: row.get(3)?, target_lang: row.get(4)?, notes: row.get(5)?,
